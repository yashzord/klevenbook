...
export function PaymentsPanel({ invoiceId, total, cancelled, payments }: { invoiceId: string; total: string; cancelled: boolean; payments: Payment[] }) {
  ...
  return (
    <section className="no-print mx-auto mt-6 max-w-[210mm] rounded-lg border border-line bg-paper p-5">
      ...
      <dl className="mb-4 grid grid-cols-3 gap-3 text-sm tabular-nums">
        <div><dt className="text-ink-soft">Invoice total</dt><dd className="font-medium">{inr(total)}</dd></div>
        <div><dt className="text-ink-soft">Received</dt><dd className="font-medium">{inr(paid)}</dd></div>
        <div><dt className="text-ink-soft">Balance due</dt><dd className={`font-semibold ${due > 0 ? 'text-amber-800' : 'text-leaf-deep'}`}>{inr(Math.max(due, 0))}</dd></div>
      </dl>
      {payments.length > 0 && (
      ...
      {cancelled ? <p className="text-sm text-ink-soft">Cancelled invoices owe nothing.</p> : due > 0 ? <PaymentForm invoiceId={invoiceId} due={due} /> : <p className="text-sm text-leaf-deep">Fully paid.</p>}
    </section>
  )
}
