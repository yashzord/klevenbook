...
export function DeleteButton({ action, label }: { action: (prev: ActionState) => Promise<ActionState>; label: string }) {
  ...
  if (!armed) return <button type="button" onClick={() => setArmed(true)} className="rounded-md px-3 py-2 text-sm text-ink-soft hover:bg-red-50 hover:text-red-700">Delete {label}</button>
  return (
