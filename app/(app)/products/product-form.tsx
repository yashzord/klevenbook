...
export function ProductForm({ product }: { product?: Product }) {
  ...
  return (
    <form key={product ? product.id : state.ok ?? 0} action={action} className="grid gap-3 rounded-lg border border-line bg-paper p-4 sm:grid-cols-6">
      ...
      <Field label="HSN"><input name="hsn" defaultValue={product?.hsn ?? ''} className={inputClass} placeholder="9018" /></Field>
      <Field label="Unit"><input name="unit" defaultValue={product?.unit} className={inputClass} placeholder="pcs" /></Field>
      <Field label="GST %"><input name="gst_rate" type="number" step="0.01" min="0" max="100" required defaultValue={product ? Number(product.gst_rate) : undefined} className={inputClass} placeholder="5" /></Field>
      <Field label="List price (₹)" className="sm:col-span-2"><input name="price" type="number" step="0.01" min="0" required defaultValue={product ? Number(product.price) : undefined} className={inputClass} placeholder="1850.00" /></Field>
